    ...
    def on_step_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        if state.global_step > 0 and effective_logging_steps > 0 and state.global_step % effective_logging_steps == 0:
            if torch.cuda.is_available():
                ...
                if trainer_instance:
                    current_device = trainer_instance.args.device # Get device from trainer args
                    ...
                    print(f"Step {state.global_step}: VRAM Usage - Allocated: {allocated:.2f}GB, Reserved: {reserved:.2f}GB (trainer not in callback)")
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if labels is not None and hasattr(outputs, "loss") and outputs.loss is not None:
            ...
            if not (hasattr(outputs, "loss") and outputs.loss is not None):
                 print("Warning: CustomSafeTrainer.compute_loss: Model did not return loss or prerequisites not met.")
...
def main():
    ...
    for split_name, config in datasets_to_process_config.items():
        ...
        def preprocess_function(examples):
            inputs_text = [] 
            prompts_text = examples["input"] # This is a list of prompts for the current batch
            outputs_text = examples["output"]

            # Tokenize all prompts in the batch once to get their lengths efficiently
            # This helps ensure consistent tokenization (e.g. BOS handling) for length calc
            tokenized_prompts_for_length_calc = tokenizer(
                prompts_text, # List of prompt strings
                max_length=args.max_seq_length,
                truncation=True,
                add_special_tokens=True # Crucial: include special tokens like BOS if tokenizer adds them
            )
            ...
            for i in range(len(prompts_text)):
                # Use the attention mask from the batched prompt tokenization to get true length
                # attention_mask[i] is a list/tensor of 1s and 0s for the i-th prompt
                prompt_tokens_length = sum(tokenized_prompts_for_length_calc['attention_mask'][i]) # Python sum on list of ints
                
                mask_len = min(prompt_tokens_length, labels.shape[1])
                labels[i, :mask_len] = -100 # labels is a tensor, this slicing is fine
            
            model_inputs["labels"] = labels
            return model_inputs
        # <<< --- END MODIFIED PREPROCESS FUNCTION --- >>>
    ...
    trainer = CustomSafeTrainer(model=model, args=training_args, train_dataset=tokenized_train_dataset, eval_dataset=tokenized_eval_dataset, tokenizer=tokenizer, data_collator=data_collator, callbacks=[memory_callback])
