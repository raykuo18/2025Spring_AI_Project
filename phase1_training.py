...
# --- Model Configuration Mapping (ensure this is up-to-date) ---
MODEL_CONFIGS = {
    "TinyLLaMA": {
...
class CustomSafeTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs): # <<< MODIFIED: Added **kwargs
        """
        ...
        Accepts **kwargs to be robust to arguments passed by Trainer.training_step.
        """
        labels = inputs.get("labels") # Use .get() for safety, though Trainer usually ensures it's there

        # Model's forward pass. If labels are present in inputs, HF models usually compute loss.
        ...
        if labels is not None and hasattr(outputs, "loss") and outputs.loss is not None:
            ...
            # Fallback: Manually compute loss if model didn't return it directly
            # (e.g., if labels were popped before passing to model, or model isn't a standard HF CausalLM with loss)
            logits = outputs.logits
            # Ensure labels are on the same device as logits for CrossEntropyLoss
            labels_on_device = labels.to(logits.device)
            
            loss_fct = torch.nn.CrossEntropyLoss() # Default ignore_index is -100
            ...
            if not (hasattr(outputs, "loss") and outputs.loss is not None):
                 print("Warning: CustomSafeTrainer.compute_loss: Model did not return loss and manual computation prerequisites (labels/logits) not fully met.")

        return (loss, outputs) if return_outputs else loss

def main():
    parser = argparse.ArgumentParser(description="Fine-tune or only tokenize for Causal LM with LoRA.")
    # ... (All your argparse arguments as defined before, including --gradient_checkpointing and --gradient_checkpointing_use_reentrant) ...
    ...
    parser.add_argument("--max_val_samples", type=int, default=None, help="Max validation samples.") # Corrected from max_eval_samples
    parser.add_argument("--max_test_samples", type=int, default=None, help="Max test samples.")
    parser.add_argument("--tokenize_only", action="store_true", help="If set, tokenize, save, and exit.")
    ...
    parser.add_argument("--gradient_checkpointing_use_reentrant", type=bool, default=False, help="Value for use_reentrant in gradient_checkpointing_kwargs (False recommended).")
    ...
    # ... (Dataset loading, tokenizing, caching, sub-sampling logic as before) ...
    ...
    if args.gradient_checkpointing and not args.load_in_4bit: # QLoRA handles this via prepare_model
        ...
        model.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": args.gradient_checkpointing_use_reentrant})
         ...
         # If prepare_model_for_kbit_training did not set use_reentrant as desired,
         # one might need to call model.gradient_checkpointing_enable again here.
         # However, use_gradient_checkpointing=True is passed to prepare_model_for_kbit_training,
         # which internally calls model.gradient_checkpointing_enable().
         # For explicit control of use_reentrant in QLoRA, one might need to call it after get_peft_model
         # if the model is already PEFT + QLoRA prepared.
         # Let's assume prepare_model_for_kbit_training with use_gradient_checkpointing=True is sufficient for now.
         # If the warning persists, one might do: model.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": args.gradient_checkpointing_use_reentrant})
         # *after* get_peft_model as well.
        pass
    ...
    # Re-check use_cache after PEFT, as get_peft_model might re-enable it or base model might have it true
    if hasattr(model, "config") and hasattr(model.config, "use_cache") and model.config.use_cache:
        ...
        model.config.use_cache = False
    elif hasattr(model, "base_model") and hasattr(model.base_model, "config") and hasattr(model.base_model.config, "use_cache") and model.base_model.config.use_cache:
        # For PEFT models, the actual config might be in model.base_model.model.config
        try:
            model.base_model.model.config.use_cache = False
            print(f"Set model.base_model.model.config.use_cache to False")
        except AttributeError:
            pass # If the structure is different
