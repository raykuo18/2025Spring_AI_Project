...
def main():
    ...
    # --- MODIFIED: Adapter Arguments for Two Branches ---
    parser.add_argument("--phase1_lora_path", type=str, default=None, help="Path to Phase 1 LoRA adapter (move prediction/rules).")
    parser.add_argument("--phase1_adapter_name", type=str, default="phase1_core", help="Internal name for Phase 1 adapter.")
    parser.add_argument("--phase2_lora_path", type=str, default=None, help="Path to Phase 2 LoRA adapter (explanations).")
    parser.add_argument("--phase2_adapter_name", type=str, default="phase2_explainer", help="Internal name for Phase 2 adapter.")
    parser.add_argument("--alpha_p1_weight", type=float, default=1.0, help="Weight for Phase 1 adapter if combining (alpha).")
    parser.add_argument("--beta_p2_weight", type=float, default=1.0, help="Weight for Phase 2 adapter if combining (beta).")
    parser.add_argument("--combination_type", type=str, default="linear", choices=["linear", "svd", "cat", "ties", "dare_ties", "dare_linear", "dare_svd"], help="PEFT combination type for add_weighted_adapter.")
    ...
    if args.phase1_lora_path:
        ...
        try:
            if not isinstance(model, PeftModel): # First adapter on base model
                model = PeftModel.from_pretrained(model, args.phase1_lora_path, adapter_name=args.phase1_adapter_name)
            else: # Already a PeftModel, add another
                model.load_adapter(args.phase1_lora_path, adapter_name=args.phase1_adapter_name)
            p1_loaded_successfully = True
        except Exception as e: print(f"Error loading Phase 1 LoRA adapter: {e}"); sys.exit(1)
    ...
    if args.phase2_lora_path:
        ...
        try:
            if not isinstance(model, PeftModel): # First adapter on base model
                model = PeftModel.from_pretrained(model, args.phase2_lora_path, adapter_name=args.phase2_adapter_name)
            else: # Already a PeftModel, add another
                model.load_adapter(args.phase2_lora_path, adapter_name=args.phase2_adapter_name)
            p2_loaded_successfully = True
        except Exception as e: print(f"Error loading Phase 1 LoRA adapter: {e}"); sys.exit(1)
    
    if p1_loaded_successfully and p2_loaded_successfully:
        # Ensure model is PeftModel before weighted adapter
        if not isinstance(model, PeftModel):
                print("Error: Model must be a PeftModel to add weighted adapter. Load at least one adapter first."); sys.exit(1)

        combined_adapter_name = f"blend_{args.phase1_adapter_name}{args.alpha_p1_weight}_{args.phase2_adapter_name}{args.beta_p2_weight}".replace(".","_") # Ensure valid name
        print(f"Creating weighted combination: '{combined_adapter_name}' (P1 w={args.alpha_p1_weight}, P2 w={args.beta_p2_weight}...")
        try:
            model.add_weighted_adapter(
                adapters=[args.phase1_adapter_name, args.phase2_adapter_name],
                weights=[args.alpha_p1_weight, args.beta_p2_weight],
                adapter_name=combined_adapter_name,
                combination_type=args.combination_type
            )
            model.set_adapter(combined_adapter_name)
        except Exception as e: print(f"Error creating weighted adapter: {e}. Defaulting to auto."); args.active_adapter_choice = "auto"
    ...
    if args.output_numerical_summary:
        ...
        try:
            ...
            with open(numerical_summary_path, "w") as f_summary:
                f_summary.write(f"Eval Summary - Model: {args.model_name} - P1 Adapter: {args.phase1_lora_path if args.phase1_lora_path else 'N/A'} - P2 Adapter: {args.phase2_lora_path if args.phase2_lora_path else 'N/A (or base if P1 also N/A)'}\n")
