...
./create_env.py --conda --path=$(pwd)/../.env --python-version=3.11
...
def main() -> None:
    ...
    create_environment(manager=manager, path=path, py_version=py_version, name=name)

    return None


def create_environment(
    manager: str, path: str, py_version: str | None, name: str | None
):
    path: str = os.path.abspath(path=path)
    command: str = env_command(
        manager=manager, path=path, py_version=py_version, name=name
    )
    ...
    if manager == "conda":
        python_cmd_exec: List[str] = shlex.split(
            s=f"{manager} activate {name or path}", comments=False, posix=True
        )
...
def env_command(
    ...
    if manager == "conda":
        if name:
            command = f"{manager} create -n {name}{py_install_version} --no-default-packages --yes"
        else:
            command = f"{manager} create -p {path}{py_install_version} --no-default-packages --yes"

    elif manager == "venv":
        if name:
            # Use name as the directory name for venv if provided
            command: str = f"python -m venv {name}"
        else:
            command: str = f"python -m venv {path}"
    else:
...
def arg_parser() -> argparse.ArgumentParser:
    ...
    optoptions.add_argument(
        ...
        type=str,
        default=None,
        help="OPTIONAL: Name of the environment directory to create. For conda, creates a named env; for venv, creates a folder with this name.",
    )
