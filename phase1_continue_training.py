...
import time 
import glob
from typing import List, Dict, Optional, Tuple # Added for completeness
...
from peft import (
    LoraConfig,
    get_peft_model,
    PeftModel, # Added for loading existing adapters
...
# --- Custom Callbacks (MemoryUsage, CheckpointMetadataLogger - as before) ---
                ...
                if trainer_instance:
                    try:
                        ...
                        if current_device.type == 'cuda':
                            allocated = torch.cuda.memory_allocated(device=current_device) / (1024**3); reserved = torch.cuda.memory_reserved(device=current_device) / (1024**3)
                            log_output = {"vram_allocated_gb": round(allocated, 2), "vram_reserved_gb": round(reserved, 2)}; trainer_instance.log(log_output)
...
def main():
    ...
    # <<< --- NEW ARGUMENT for resuming LoRA adapter --- >>>
    parser.add_argument("--resume_lora_adapter_path", type=str, default=None,
                        help="Path to an existing LoRA adapter to continue training. If provided, this adapter is loaded.")
    # <<< --- END NEW ARGUMENT --- >>>
    ...
    parser.add_argument("--lora_r", type=int, default=8, help="LoRA rank (used if not resuming adapter or for new adapter in P2).")
    parser.add_argument("--lora_alpha", type=int, default=32, help="LoRA alpha (used if not resuming adapter).")
    parser.add_argument("--lora_dropout", type=float, default=0.1, help="LoRA dropout (used if not resuming adapter).")
    ...
    parser.add_argument("--checkpoint_metadata_file_name", type=str, default="checkpoint_log.txt", help="Filename for checkpoint metadata log within run directory.")
    ...
    # --- Create Unique Run Directory (as before) ---
    if args.run_name:
        ...
        run_specific_output_dir = os.path.join(args.output_dir, f"{run_name_prefix}_{timestamp}")
    ...
    target_modules_for_new_lora = model_config_details["target_modules"] # Used if not resuming adapter
    ...
    # --- 2. Dataset Preparation (as before) ---
    # ... (Full dataset loading, tokenizing, caching, sub-sampling logic as in the previous version) ...
    ...
    print(f"Final train dataset size: {len(tokenized_train_dataset) if tokenized_train_dataset else 0}") # Print final sizes
    ...
    if not effective_output_dir_for_trainer: parser.error("--output_dir (for run folder) is effectively required."); sys.exit(1)
    ...
    # --- LoRA Setup: Load existing adapter OR create new one ---
    if args.resume_lora_adapter_path:
        print(f"Loading existing LoRA adapter from: {args.resume_lora_adapter_path} to continue training...")
        if not os.path.isdir(args.resume_lora_adapter_path):
            print(f"Error: --resume_lora_adapter_path '{args.resume_lora_adapter_path}' not found or not a directory.")
            sys.exit(1)
        try:
            model = PeftModel.from_pretrained(model, args.resume_lora_adapter_path, is_trainable=True)
            print("Existing LoRA adapter loaded and set as trainable.")
        except Exception as e:
            print(f"Error loading LoRA adapter from {args.resume_lora_adapter_path}: {e}")
            print("Ensure the path points to a valid PEFT adapter directory (containing adapter_config.json and adapter_model.*).")
            sys.exit(1)
    else:
        print("No existing LoRA adapter provided. Setting up new LoRA configuration...")
        current_target_modules = MODEL_CONFIGS[args.model_name]["target_modules"] # Use modules for the current model
    ...
    # Ensure use_cache is False after PEFT wrapping/loading
    if hasattr(model, "config") and hasattr(model.config, "use_cache") and model.config.use_cache: model.config.use_cache = False
    elif hasattr(model, "base_model") and hasattr(model.base_model, "model") and hasattr(model.base_model.model, "config") and \
         hasattr(model.base_model.model.config, "use_cache") and model.base_model.model.config.use_cache:
        try: model.base_model.model.config.use_cache = False; print("Set model.base_model.model.config.use_cache to False")
        except AttributeError: pass

    if args.gradient_checkpointing : # Handles both QLoRA (via prepare_model) and non-QLoRA
        # For non-QLoRA, this enables it. For QLoRA, prepare_model_for_kbit_training already did.
        # Re-enabling on PEFT model ensures it's set with correct kwargs if not done by prepare_...
        # or if model was not QLoRA prepared but is now PEFT.
        if not (args.load_in_4bit and model.is_gradient_checkpointing): # Avoid re-enabling if QLoRA already did
    ...
    training_args_dict = { # Using effective_output_dir_for_trainer
        "output_dir":effective_output_dir_for_trainer, "num_train_epochs":args.num_train_epochs,
        "per_device_train_batch_size":args.per_device_train_batch_size, "per_device_eval_batch_size":args.per_device_eval_batch_size if tokenized_eval_dataset else args.per_device_train_batch_size,
        ...
        "optim":args.optim, "logging_dir":os.path.join(effective_output_dir_for_trainer, "hf_trainer_logs"), "logging_steps":args.logging_steps,
    ...
    if args.checkpoint_metadata_file_name and not args.tokenize_only:
        ...
        with open(metadata_log_filepath, initial_log_mode, encoding="utf-8") as f:
            if initial_log_mode == "w" or (os.path.exists(metadata_log_filepath) and os.path.getsize(metadata_log_filepath) == 0) :
                 f.write(f"CKPT Log - Run: {os.path.basename(effective_output_dir_for_trainer)} @ {time.strftime('%Y%m%d-%H%M%S')}\n"); [f.write(f"  {k}: {v}\n") for k, v in sorted(vars(args).items())]; f.write("---\n\n")
    ...
    # --- Handle Resume from Trainer Checkpoint vs. Just LoRA Weights ---
    resume_from_trainer_checkpoint = None
    if args.resume_lora_adapter_path and os.path.exists(os.path.join(args.resume_lora_adapter_path, "trainer_state.json")):
        # If the resume_lora_adapter_path looks like a full Trainer checkpoint, we can use it to resume Trainer state
        # However, user wants a NEW LR scheduler, so we should NOT pass resume_from_checkpoint path to trainer.train()
        # if that would load optimizer/scheduler state.
        # The LoRA weights are already loaded.
        print(f"LoRA weights loaded from {args.resume_lora_adapter_path}. Training will start with new optimizer/scheduler as per current TrainingArguments.")
    
    print("Starting training..."); trainer.train(resume_from_checkpoint=None); print("Training complete.") # Explicitly None
    
    if tokenized_test_dataset:
        print("\nEvaluating on test set..."); test_results = trainer.evaluate(eval_dataset=tokenized_test_dataset, metric_key_prefix="test")
        print(f"Test Metrics: {test_results}"); test_results_path = os.path.join(effective_output_dir_for_trainer, "test_results.json")
        with open(test_results_path, "w") as f: json.dump(test_results, f, indent=4); print(f"Test results saved: {test_results_path}")
    
    final_adapter_path = os.path.join(effective_output_dir_for_trainer, "final_lora_adapter") # Consistent name
    print(f"Saving final LoRA adapter to {final_adapter_path}..."); model.save_pretrained(final_adapter_path)
    print("LoRA adapter saved."); print(f"Base model: {hf_model_name}")
